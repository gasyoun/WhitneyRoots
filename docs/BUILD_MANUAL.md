...
> - *What the data means, schema* → [DESIGN.md](https://github.com/gasyoun/WhitneyRoots/blob/main/DESIGN.md); *the authority order* is stated in [REVIEWER_GUIDE.md](https://github.com/gasyoun/WhitneyRoots/blob/main/REVIEWER_GUIDE.md) and the [CHANGELOG.md](https://github.com/gasyoun/WhitneyRoots/blob/main/CHANGELOG.md) header.
...
```
 ...
 [0] scratch/phase0/parse_warnemyr.py ──► scratch/phase0/root_spine.json   (+ audit.md)
                                                │ (the mutable hub — tracked and committed)
...
| File | Role | Producer |
|---|---|---|
| `scratch/phase0/root_spine.json` | Crosswalk hub (Track A). Tracked and committed; Stage 0 regenerates it in place. | [scratch/phase0/parse_warnemyr.py](https://github.com/gasyoun/WhitneyRoots/blob/main/scratch/phase0/parse_warnemyr.py), then mutated by stages 1–4 |
...
- **Mode flags**: `--pilot` (chapters X + XIII, the default) · `--full` (verb-system chapters IX–XV — the live-build choice) · `--all` (every chapter I–XVIII) are mutually exclusive; `--chapters X,XIII,XI` (explicit comma-separated Roman numerals) is a separate flag that **silently wins** if combined with any of them.
...
| [scripts/bundle.js](https://github.com/gasyoun/WhitneyRoots/blob/main/scripts/bundle.js) | JS bundle concatenator (Node) | Hardcoded 17-file order (vendored sanskrit-util added 1.5.0); missing files warn + skip, not fail |
...
Two numbering systems coexist: [CHANGELOG.md](https://github.com/gasyoun/WhitneyRoots/blob/main/CHANGELOG.md) carries public **semantic versions** (1.0.0 = the pipeline + revert story; 1.1.0/1.2.0 = the accent-rule table + VedaWeb validation; 1.4.0 = this manual itself; 1.5.0/1.5.1 = the alternation-type authorial ingest + the sanskrit-util JS re-vendor, then the homonym-smear fix), while [.ai_state.md](https://github.com/gasyoun/WhitneyRoots/blob/main/.ai_state.md) tracks internal **Phase numbers** (the Phase-8 revert lives there and in [CLAUDE.md](https://github.com/gasyoun/WhitneyRoots/blob/main/CLAUDE.md), *not* in the changelog). Org-wide `H###` handoff ids referenced in commit messages resolve in the external [Uprava handoffs registry](https://github.com/gasyoun/Uprava/blob/main/handoffs/README.md), not in this repo.
