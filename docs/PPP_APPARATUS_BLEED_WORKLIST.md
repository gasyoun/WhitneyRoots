...
> **✅ RESOLVED 2026-06-14 — all recommended defaults accepted (PRESERVE).** The cleanup [`scripts/dcs/fix_ppp_apparatus_bleed.py`](../scripts/dcs/fix_ppp_apparatus_bleed.py) strips apparatus out of every `ppp` string and preserves it in three **additive, consumer-safe** sidecar fields:
> - **(a)** period/source markers + period digits → `ppp_attestation` `{form: [markers]}`.
> - **(b)** Whitney `?` → `ppp_uncertain` `[forms]` (the 13 forms in §2).
> - **(c)** `&` always split; bare space split only for the confirmed variant pair id 747 (`vrdha`/`brdha`) — no blanket split-on-space.
> - **(d)** `= seq.`/`adj` → `ppp_note` `{form: note}`; id 74 `ardita =` keeps a **dangling-`=`** note flagging the truncated cross-ref for a human.
> - **(e)** id 649 → `["rista", "rise"]`; the OCR-doubled `rises` is dropped (recorded in the audit).
>
> Full before/after + every stripped marker is in [`docs/ppp_apparatus_bleed_audit.json`](ppp_apparatus_bleed_audit.json). These resolved policy decisions are recorded **here + in the audit** rather than in [`DECISIONS_NEEDED.md`](DECISIONS_NEEDED.md): that file is machine-generated by [`scripts/build_decisions_doc.py`](../scripts/build_decisions_doc.py) and carried unrelated in-flight `§3e` edits in the tree at fix time, so regenerating it would have entangled that work into this data-only change.
...
- [x] All **39** ids in §2 carry their **verified canonical** `ppp` (apparatus stripped, alternates split, `rises` dropped).
- [x] No `ppp` string in any of the 39 contains: a period/source marker (`RV1 RV2 E1 C1 S1 B1 K R AA` or a trailing bare digit), `?`, `&`, `=`, `adj`, or `seq.`.
- [x] id 332 → length 3; id 452 → length 2; ids 469/470/471 → length 3; id 747 → length 2; id 649 → length 2. (Length changes are intentional; analytics centrality updates accordingly.)
- [x] [`src/app_data.json`](../src/app_data.json) is valid JSON, **BOM-free**, **CRLF**, **935 entries**, **no trailing newline** — byte-format identical to before outside the edited spans.
- [x] Script is **idempotent** (second run = "Nothing to change").
- [x] If decisions §4(a)/(b)/(d) accepted: `ppp_attestation` / `ppp_uncertain` / `ppp_note` populated and additive (no consumer regression).
- [x] detail.js "Whitney PPP vs corpus" chips for the affected roots no longer show false ✗ caused by apparatus suffixes (verified in the running app).
- [x] The 6 **gloss-bleed** records (19, 100, 182, 184, 347, 831) are **untouched**.
